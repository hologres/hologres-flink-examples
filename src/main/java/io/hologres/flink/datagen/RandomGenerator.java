...
public abstract class RandomGenerator<T> implements DataGenerator<T> {
    ...
    public static RandomGenerator<BigDecimal> decimalRandomGenerator(int scale, int percision) {
        return new RandomGenerator<BigDecimal>() {
            @Override
            public BigDecimal next() {
                return new BigDecimal(random.nextInt(0, 1000));
            }
        };
    }

    public static RandomGenerator<LocalDate> dateGenerator() {
        return new RandomGenerator<LocalDate>() {
            @Override
